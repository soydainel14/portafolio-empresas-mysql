...
async function initDb() {
  ...
  // Seed/ensure admin (si defines ADMIN_EMAIL y ADMIN_PASSWORD, fuerza el password)
// Esto evita el problema de "Credenciales inválidas" cuando ya había un admin viejo en la BD.
  const seedEmail = t(process.env.ADMIN_EMAIL).toLowerCase();
  const seedPass = String(process.env.ADMIN_PASSWORD || "");
  ...
  if (seedEmail && seedPass) {
    ...
    if (!existing) {
      ...
      logJson("info", "admin.seed.created", { email: seedEmail });
    } else {
      await pool.execute(
        `UPDATE admin_users SET password_hash = ?, role = 'admin' WHERE id = ?`,
        [hash, existing.id]
      );
      logJson("info", "admin.seed.updated", { email: seedEmail });
    }
  } else {
    // Si no hay ningún admin y tampoco hay env vars, avisa
...
(async () => {
  try {
    await initDb();
    app.listen(port, "0.0.0.0", () => console.log(`✅ Portafolio Empresas corriendo en puerto ${port}`));
  } catch (err) {
